...
# GHOST: the override and signal-status sentences above are local (MODS.md)
#
...
fm_timeout_mechanism() {
  # GHOST: the override accepts every mechanism, not only bash (MODS.md)
...
fm_run_timed() {  # <seconds> <command...>
  ...
  case "$(fm_timeout_mechanism)" in
    ...
    perl)
      # GHOST: a signal-killed command exits 128 + signal, not 0 (MODS.md)
